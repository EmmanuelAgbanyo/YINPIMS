...
export const getBadgeTitleTheme = (type?: ParticipantBadgeType) => {
  switch (type) {
    case 'Coordinator':
      return { bg: 'bg-gradient-to-r from-blue-600 to-indigo-700', text: 'text-white', label: 'COORDINATOR', border: 'border-blue-500' };
    case 'Speaker':
      ...
      <div className={`bg-[#FAFAF9] border border-[#E4E4E1] rounded-2xl shadow-2xl w-full ${badgeOrientation === 'landscape' ? 'max-w-2xl' : 'max-w-md'} flex flex-col my-8 overflow-hidden transition-all duration-200`}>
        ...
        <div className="no-print p-4 bg-white border-b border-[#E4E4E1] space-y-3">
          ...
          <div className="space-y-1.5 pt-2.5 border-t border-[#E4E4E1]">
            ...
            <div className="flex flex-wrap gap-1.5">
              {(['Delegate', 'Contestant', 'Speaker', 'Volunteer', 'Staff', 'Coordinator'] as ParticipantBadgeType[]).map(t => (
                <button
                  key={t}
        ...
        <div className="p-6 flex flex-col items-center justify-center space-y-4">
          {badgeOrientation === 'portrait' ? (
            ...
            <div
              ref={badgeRef}
              className="printable-badge-container landscape-badge w-full max-w-xl bg-white border-2 border-[#E4E4E1] rounded-2xl shadow-xl overflow-hidden relative flex flex-col justify-between"
            >
              {/* Badge Top Header */}
              <div className="bg-[#14595A] text-white p-4 text-center space-y-1" style={{ WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>
                <div className="flex items-center justify-center space-x-1 text-[10px] font-bold uppercase tracking-widest text-white/80">
                ...
                <h3 className="font-heading font-bold text-base leading-tight text-white px-4">
                  {event.name}
                </h3>
                <p className="text-[11px] text-white/80 font-medium">
                  {new Date(event.startDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
              ...
              <div className="p-6 bg-white flex-1 flex items-center justify-between gap-6">
                {/* Left Column: Participant Details */}
                <div className="flex-1 min-w-0 flex flex-col justify-center space-y-3 text-left">
                  <div>
                    <h2 className="font-heading font-bold text-2xl text-[#1C1C1A] tracking-tight leading-snug">
                      {participant.fullName}
                    </h2>

                    {effectiveInstitution && (
                      <div className="mt-2 inline-flex items-center space-x-1.5 px-3 py-1 bg-[#EBF4F4] text-[#14595A] rounded-full border border-[#14595A]/20 max-w-full" style={{ WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>
                        <Building className="h-3.5 w-3.5 shrink-0" />
                    ...
                    {participant.jobTitle && (
                      <p className="text-[11px] text-[#6B6B66] mt-1 font-medium truncate">
                        {participant.jobTitle}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="space-y-1.5 pt-2 border-t border-[#F0F0EE]">
                    ...
                    {room && (
                      <div className="inline-flex items-center space-x-1.5 px-3 py-1 rounded-full bg-[#EBF4F4] text-[#14595A] text-xs font-bold w-fit" style={{ WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>
                        <Bed className="h-3.5 w-3.5 shrink-0" />
                        <span>Assigned Room: {room.roomNumber} ({room.genderGroup} Wing)</span>
                      </div>
                    )}
                  </div>
                </div>

                {/* Right Column: QR Code & Test Pass */}
                <div className="shrink-0 flex flex-col items-center justify-center pl-6 border-l border-[#E4E4E1]">
                  <a
                    href={passUrl}
                    target="_blank"
                    rel="noopener noreferrer"
              ...
              {/* Badge Footer */}
              <div className="bg-[#FAFAF9] px-6 py-2.5 border-t border-[#E4E4E1] flex items-center justify-between text-[10px] text-[#6B6B66]" style={{ WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>
                <span className="font-semibold">PIMS ID Pass System</span>
                <span className="font-semibold text-[#2F7D4F] flex items-center space-x-1">
                  <span className="h-2 w-2 rounded-full bg-[#2F7D4F]" style={{ WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }} />
                  <span>ACTIVE</span>
